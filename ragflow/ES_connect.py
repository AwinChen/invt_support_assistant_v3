...
class ESConnection(DocStoreConnection):
    def __init__(self):
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                time.sleep(5)
        if not self.es.ping():
            msg = f"Elasticsearch {self.es_host} is unhealthy in 120s."
            raise Exception(msg)
    ...
    def delete(self, condition: dict, indexName: str, knowledgebaseId: str) -> int:
        ...
        if "id" in condition:
            ...
            if not chunk_ids:  # when chunk_ids is empty, delete all
                # qry = Q("match_all")
                raise ValueError("必须给出id编号，才可以删除！")
            else:
    ...
    # 清除ES向量库中指定indexName的全部数据（一定要确认后才可使用）
    def delete_the_indexName_all_data(self, indexName: str):
        try:
            # 使用 match_all 查询删除所有文档
            response = self.es.delete_by_query(
                index=indexName,
                body={"query": {"match_all": {}}}
            )
            print(f"✅ 索引 {indexName} 清空成功！删除文档数: {response['deleted']}")
        except Exception as e:
            print(f"❌ 清空失败: {e}")
    
    # 清除ES向量库中指定indexName，其会直接删除（一定要确认后才可使用）
    def delete_indexName_in_es_storage(self, indexName: str):
        try:
            # 删除索引
            self.es.indices.delete(index=indexName)
            print(f"✅ 索引 {indexName} 删除成功！")
        except NotFoundError:
            print(f"❌ 索引 {indexName} 不存在")
        except Exception as e:
            print(f"❌ 删除失败: {e}")
...
if __name__ == "__main__":
    es_indexname = "invt_gd5000_img_demo"
    # ESConnection().delete_the_indexName_all_data(indexName=es_indexname)
    # ESConnection().delete_indexName_in_es_storage(indexName=es_indexname)
